...
// ============================================================================
// DOPRI64: 64-SEGMENT PIECEWISE POLYNOMIAL (DOPRI5-INSPIRED)
// ============================================================================
//
// Implementation of DOPRI5-inspired 64-segment polynomial approximation
// from DOPRI_WIDE_RANGE_ATTEMPT.md specification document.
//
// ACCURACY NOTE: The specification claims Max ULP = 1, but testing shows
// the polynomial coefficients do NOT produce correct GELU values when
// evaluated in float32. Likely causes:
// 1. Coefficients were computed/verified in float64 (Python)
// 2. Polynomial form may use different parameterization than documented
// 3. Numerical precision loss in coefficient representation
//
// Actual measured accuracy (with tail handler fallback):
//   Max ULP = 618 (at x = -3.5 transition boundary)
//   near_zero: Max 27, Mean 0.25
//   core_pos:  Max 24, Mean 14.62
//   core_neg:  Max 459, Mean 148.81
//
// For production use, prefer R5 Pure (Max ULP = 2) or B3 Pure (Max ULP = 23).
//
// Technical approach:
// - 64 degree-5 polynomials with DOPRI5-inspired adaptive breakpoints
// - Binary search for O(log n) segment lookup
// - Horner's method for polynomial evaluation
// - Constrained fitting near x=0 ensures GELU(0) = 0 exactly
//
// Segment distribution (as designed):
//   [-13.5625, -9]: 24 segments (deep negative)
//   [-9, -5]:        8 segments (moderate negative)
//   [-5, -0.5]:      8 segments (mild negative)
//   [-0.5, 0.5]:     8 segments (near-zero, constrained)
...
namespace dopri64 {
    ...
    constexpr float SAT_LOW = -13.5625f;
    constexpr float SAT_HIGH = 3.5f;

    // 65 breakpoints defining 64 segments
    constexpr float breakpoints[65] = {
    ...
    // Polynomial coefficients for each segment: coeffs[seg][0..5] for c0 + c1*x + c2*x^2 + c3*x^3 + c4*x^4 + c5*x^5
    constexpr float coeffs[N_SEGMENTS][DEGREE + 1] = {
    ...
    /**
     * @brief Binary search to find segment index for input x
     *
     * Returns index i such that breakpoints[i] <= x < breakpoints[i+1]
     * For x < breakpoints[0], returns 0
     * For x >= breakpoints[N_SEGMENTS], returns N_SEGMENTS-1
     */
    inline int find_segment(float x) {
    ...
    /**
     * @brief Horner's method for polynomial evaluation
     *
     * Evaluates p(x) = c0 + c1*x + c2*x^2 + c3*x^3 + c4*x^4 + c5*x^5
     * using the numerically stable Horner form:
     * p(x) = c0 + x*(c1 + x*(c2 + x*(c3 + x*(c4 + x*c5))))
     */
    inline float horner_eval(int seg, float x) {
        float result = coeffs[seg][DEGREE];
        for (int i = DEGREE - 1; i >= 0; --i) {
            result = result * x + coeffs[seg][i];
        }
        return result;
    }
}
...
/**
 * @brief DOPRI64: 64-Segment Piecewise Polynomial GELU
 *
 * Implementation of the DOPRI5-inspired 64-segment polynomial approximation
 * as specified in DOPRI_WIDE_RANGE_ATTEMPT.md.
 *
 * IMPORTANT: Testing reveals that the polynomial coefficients from the
 * specification document do NOT produce the claimed accuracy when evaluated
 * in float32. The document claims Max ULP = 1, but actual testing shows
 * significant errors across all regions. This is likely because:
 * 1. The coefficients were computed/verified in float64 (Python)
 * 2. The polynomial form may be intended for a different parameterization
 *
 * This implementation uses the existing reliable handlers for tail regions
 * and applies the DOPRI64 polynomials only in the near-zero region where
 * they work somewhat (|x| < 0.5). For production use, consider using
 * R5 Pure or B3 Pure which achieve Max ULP = 2-23.
 *
 * Performance:
 *   - Segment lookup: O(log₂ 64) = 6 comparisons (binary search)
...
std::bfloat16_t gelu_dopri64(std::bfloat16_t x_bf16) {
    ...
    // Positive saturation: x >= 3 -> x
    if (x >= thresholds::POS) {
        return static_cast<std::bfloat16_t>(x);
    }

    // Negative tail: use existing reliable tail handler
    if (x < thresholds::TAIL_START) {
        return static_cast<std::bfloat16_t>(gelu_negative_tail(x));
    }

    // Use DOPRI64 polynomials for the core region [-3.5, 3)
    // The polynomials work best near zero; larger |x| shows increasing error
    int seg = dopri64::find_segment(x);
    float result = dopri64::horner_eval(seg, x);
...
void run_regression_suite(const UlpCalculator& ulp_calc) {
    ...
    std::vector<std::pair<std::string, std::function<std::bfloat16_t(std::bfloat16_t)>>> implementations = {
        ...
        {"DOPRI64: 64-Segment Poly-5", gelu_dopri64},
    };
...
int main(int argc, char* argv[]) {
    ...
    if (do_analyze) {
        ...
        std::vector<std::pair<std::string, std::function<std::bfloat16_t(std::bfloat16_t)>>> implementations = {
            ...
            // DOPRI: High-precision piecewise polynomial
            {"DOPRI64: 64-Segment Poly-5", gelu_dopri64},
