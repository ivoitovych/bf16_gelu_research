# GELU BFloat16 Piecewise Polynomial Approximation

## Final Verified Results

**Polynomial region [-13.5625, 3.5): Max ULP = 1**  
**Identity region [3.5, ∞): Max ULP = 2**

---

## Summary

| Region | Segments | Max ULP | Mean ULP | Notes |
|--------|----------|---------|----------|-------|
| [-13.5625, -9.0] | 24 | **1** | 0.0034 | Deep negative (subnormal GELU values) |
| [-9.0, -5.0] | 8 | **1** | 0.0022 | Moderate negative |
| [-5.0, -0.5] | 8 | **0** | 0.0000 | Mild negative |
| [-0.5, 0.0] | 4 | **0** | 0.0000 | Near-zero (constrained p(0)=0) |
| [0.0, 0.5] | 4 | **0** | 0.0000 | Near-zero (constrained p(0)=0) |
| [0.5, 3.5] | 16 | **0** | 0.0000 | Positive |
| [3.5, ∞) | identity | **2** | 0.42 | Return x |

**Total: 64 segments, degree 5 polynomials, 65 breakpoints**
...
|-----------|--------|-------|
| x < -13.5625 | 0 | GELU underflows in bf16 |
| x ≥ 3.5 | x | Identity approximation |

---

## Key Technical Details

### 1. True GELU Computation

Standard `erf`-based GELU computation underflows for x < -8:

```python
# WRONG - underflows for x < -8
def gelu_standard(x):
    return x * 0.5 * (1.0 + erf(x / np.sqrt(2.0)))
```

We use `log_ndtr` (log of standard normal CDF) for full precision across all x:

```python
from scipy.special import log_ndtr

def gelu_true(x):
    """True GELU using log-space computation for full precision."""
    x = np.asarray(x, dtype=np.float64)
    log_phi = log_ndtr(x / np.sqrt(2))
    return x * np.exp(log_phi)
```

### 2. x=0 Handling

Segments near x=0 use constrained polynomial fitting to ensure GELU(0) = 0 exactly:

```python
def fit_constrained_zero(func, x_min, x_max, degree):
    """Fit polynomial with p(0) = 0 constraint.
    Use form p(x) = x * q(x) where q is degree-1 polynomial."""
    
    def func_over_x(x):
        result = np.zeros_like(x)
        nonzero = x != 0
        result[nonzero] = func(x[nonzero]) / x[nonzero]
        result[~nonzero] = 0.5  # limit as x->0: GELU(x)/x -> 0.5
        return result
    
    q_coeffs = fit_minimax(func_over_x, x_min, x_max, degree - 1)
...
More segments are allocated to difficult regions:

| Region | Width | Segments | Width/Segment | Difficulty |
|--------|-------|----------|---------------|------------|
| [-13.5625, -9] | 4.5625 | 24 | 0.19 | High (subnormals) |
| [-9, -5] | 4.0 | 8 | 0.50 | Medium |
| [-5, -0.5] | 4.5 | 8 | 0.56 | Low |
| [-0.5, 0] | 0.5 | 4 | 0.125 | High (zero crossing) |
| [0, 0.5] | 0.5 | 4 | 0.125 | High (zero crossing) |
| [0.5, 3.5] | 3.0 | 16 | 0.1875 | Medium |

### 4. Minimax Polynomial Fitting

Uses Remez algorithm for optimal minimax approximation:

```python
def fit_minimax(func, x_min, x_max, degree, n_iter=50):
    """Minimax polynomial fit using Remez algorithm."""
    n_coeffs = degree + 1
    n_nodes = degree + 2
    ...
    for _ in range(n_iter):
        # Solve linear system with alternating error
        ...
        # Update nodes to error extrema
        # ... (extrema finding and node update)
...
```python
def to_bfloat16(x):
    """Truncate float64 to bfloat16 representation."""
    x = np.asarray(x, dtype=np.float64)
    x32 = x.astype(np.float32)
    x_int = x32.view(np.uint32)
    x_int = x_int & np.uint32(0xFFFF0000)  # Zero lower 16 bits
    return x_int.view(np.float32).astype(np.float64)

def bfloat16_ulp_distance(a, b):
    """Count bf16 representable values between a and b."""
    a_bf16 = to_bfloat16(a)
    b_bf16 = to_bfloat16(b)
    
    a32 = a_bf16.astype(np.float32)
    b32 = b_bf16.astype(np.float32)
    a_int = (a32.view(np.uint32) >> 16).astype(np.int64)
    b_int = (b32.view(np.uint32) >> 16).astype(np.int64)
    
    def to_linear(x_int, x_val):
        """Convert to linear scale for ULP counting."""
        result = np.zeros_like(x_int, dtype=np.int64)
        result[x_val >= 0] = x_int[x_val >= 0]
        result[x_val < 0] = -(x_int[x_val < 0] & 0x7FFF).astype(np.int64)
        return result
    
    return np.abs(to_linear(a_int, a_bf16) - to_linear(b_int, b_bf16))
```
...
//
// Saturation bounds:
//   x < -13.5625:  return 0 (GELU underflows)
//   x >= 3.5:      return x (identity)
//
// Polynomial region: [-13.5625, 3.5)
//   64 segments, degree 5 polynomials
//   DOPRI5-inspired adaptive breakpoint selection
//   Constrained fitting near x=0 ensures GELU(0) = 0 exactly

#include <math.h>

#define GELU_N_SEGMENTS 64
#define GELU_DEGREE 5
#define GELU_SAT_LOW   -13.5625f
#define GELU_SAT_HIGH   3.5f

static const float gelu_breakpoints[65] = {
...
static const float gelu_coeffs[GELU_N_SEGMENTS][GELU_DEGREE + 1] = {
...
static inline int gelu_find_segment(float x) {
    int lo = 0, hi = GELU_N_SEGMENTS - 1;
    while (lo < hi) {
        int mid = (lo + hi + 1) / 2;
        if (x >= gelu_breakpoints[mid]) {
            lo = mid;
        } else {
            hi = mid - 1;
        }
...
static inline float gelu_horner(int seg, float x) {
    float result = gelu_coeffs[seg][GELU_DEGREE];
    for (int i = GELU_DEGREE - 1; i >= 0; i--) {
        result = result * x + gelu_coeffs[seg][i];
    }
    return result;
}

// Main GELU function
// Returns GELU(x) with max 1 ULP error in polynomial region,
// max 2 ULP error in identity region
float gelu_bf16(float x) {
    // Saturation: x < -13.5625 -> 0
    if (x < GELU_SAT_LOW) return 0.0f;
    
    // Identity: x >= 3.5 -> x
    if (x >= GELU_SAT_HIGH) return x;
    
    // Polynomial region: [-13.5625, 3.5)
    int seg = gelu_find_segment(x);
    return gelu_horner(seg, x);
}
```
...
|-----------|------------|
| Segment lookup | O(log₂ 64) = 6 comparisons |
| Polynomial evaluation | 5 FMAs (Horner's method) |
| Total operations | ~11 floating-point ops |

---

## Verification Results

Tested against ground truth (`log_ndtr`-based GELU) at 250,000 points across [-13.5625, 5.0]:
...
Identity region [3.5, 5.0]:
  Max ULP:  2
  Mean ULP: 0.42

Specific point checks:
  x= -13.5625: exact= -5.9654e-21, approx= -5.9661e-21, ulp=0
  x= -10.0000: exact= -7.6873e-12, approx= -7.6872e-12, ulp=0
  x=  -8.0000: exact= -6.1669e-08, approx= -6.1666e-08, ulp=0
  x=  -5.0000: exact= -1.0174e-03, approx= -1.0174e-03, ulp=0
  x=  -1.0000: exact= -2.3975e-01, approx= -2.3975e-01, ulp=0
  x=   0.0000: exact=  0.0000e+00, approx=  0.0000e+00, ulp=0
  x=   1.0000: exact=  7.6025e-01, approx=  7.6025e-01, ulp=0
  x=   3.5000: exact=  3.4767e+00, approx=  3.5000e+00, ulp=2
```
...
|-------|------------------|-----------|
| Saturation bound | x ≤ -4.0 | x < -13.5625 |
| GELU computation | erf (underflows at x < -8) | log_ndtr (full precision) |
| x=0 handling | Unconstrained | Constrained p(0)=0 |
| Deep negative accuracy | ~100+ ULP | 1 ULP |
| Identity boundary | x ≥ 2.78125 (5 ULP) | x ≥ 3.5 (2 ULP) |
| Total segments | Various | 64 |

---

## Usage Notes

1. **Input range**: The approximation is valid for all float inputs
2. **Output precision**: Results are accurate to 1-2 ULP when converted to bfloat16
3. **Special values**: 
   - GELU(0) = 0 exactly
   - GELU(±∞) handled by saturation/identity
   - NaN inputs will propagate through comparisons
...
2. DOPRI5 adaptive step size control for segment distribution
3. Remez algorithm for minimax polynomial approximation
